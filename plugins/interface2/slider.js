	...
	$.fn.slider = function(options) {
		options = $.extend({
			keyup: function(e) {
				if (/^35$|36$|38$|39$|40$|37$/.test(e.keyCode)) {
					...
					switch(e.keyCode) {
						...
						case 35:
							if (this.DB.axis != 'y') {
								position.x = this.DB.limitOffset.x + this.DB.limitOffset.w - this.DB.size.wb;
							}
							if (this.DB.axis != 'x') {
								position.y = this.DB.limitOffset.y + this.DB.limitOffset.h - this.DB.size.hb;
							}
							break;
						//home
						case 36:
						...
						case 40:
							if (this.DB.axis != 'x') {
								position.y = Math.min(this.DB.limitOffset.y + this.DB.limitOffset.h - this.DB.size.hb,
												this.DB.ticks && ( this.DB.ticksOn || e.ctrlKey||e.shiftKey||e.altKey) ?
												(parseInt(position.y/this.DB.grid.y,10) + 1) * this.DB.grid.y
												:  position.y + 1);
							}
						...
						case 39:
							if (this.DB.axis != 'y') {
								position.x = Math.min(this.DB.limitOffset.x + this.DB.limitOffset.w - this.DB.size.wb,
												this.DB.ticks && ( this.DB.ticksOn || e.ctrlKey||e.shiftKey||e.altKey) ?
												(parseInt(position.x/this.DB.grid.x,10) + 1) * this.DB.grid.x
												:  position.x + 1);
							}
			...
			moveThumb: function(e, position) {
				...
				var xproc = parseInt((position.x/(this.DB.maximum.w - this.DB.size.wb))*100, 10)||0;
				var yproc = parseInt((position.y/(this.DB.maximum.h - this.DB.size.hb))*100, 10)||0;
			...
			getOffset : function() {
				...
				this.DB.maximum = {
					w: $.iUtil.getInnerWidth(this),
					h: $.iUtil.getInnerHeight(this)
				};
				this.DB.limitOffset = {
					x: 0,
					y: 0,
				...
				if (this.DB.restricted) {
					...
					if (nextThumb) {
						if (this.DB.axis != 'y') {
							this.DB.limitOffset.w = nextThumb.offsetLeft + this.DB.size.wb;
						}
						if (this.DB.axis != 'x') {
							this.DB.limitOffset.h = nextThumb.offsetTop + this.DB.size.hb;
						}
					}
				...
				return {
					x: Math.max(
						...
						Math.min(
							...
							el.DB.limitOffset.x + el.DB.limitOffset.w - el.DB.size.wb
						)) - el.DB.offset.x,
					y: Math.max(
						el.DB.limitOffset.y,
						Math.min(
							el.DB.offset.y + delta.y,
							el.DB.limitOffset.y + el.DB.limitOffset.h - el.DB.size.hb
						))-el.DB.offset.y
				};
			},
			drag: function(e) {
			...
			stopDrag: function() {
				...
				var xproc = parseInt((thumbOffset.x/(this.DB.maximum.w - this.DB.size.wb))*100, 10)||0;
				var yproc = parseInt((thumbOffset.y/(this.DB.maximum.h - this.DB.size.hb))*100, 10)||0;
			...
			if (el.DB && el.DB.thumbs) {
				...
				el.DB.thumbs.each(function(nr){
					values[values.length] = {
						x: parseInt((this.offsetLeft / (el.DB.maximum.w - this.offsetWidth)) * 100, 10)||0,
						y: parseInt((this.offsetTop / (el.DB.maximum.h - this.offsetHeight)) * 100, 10)||0
					};
				});
			}
			return values;
