...
if(Ext.dd.DragDropMgr){
  Ext.dd.DragDropMgr.clickTimeThresh = 350;
}
...
Ext.SplitBar = function(dragElement, resizingElement, orientation, placement, existingProxy){
    ...
    this.dd = new Ext.dd.DDProxy(this.el.dom.id, "SplitBars", {dragElId : this.proxy.id});
...
Ext.SplitBar.createShim = function(){
    var shim = new Ext.Element(document.createElement("div"));
    shim.unselectable();
    shim.setStyle({
        "width": "100%",
        "height": "100%",
    ...
    document.body.appendChild(shim.dom);
    return shim;
};
...
Ext.QuickTips = function(){
    ...
    return {
       init : function(){
          ...
          cfg = tm.tagConfig;
          if(!inited){
              el = new Ext.Layer({cls:"x-tip", shadow:"sides", useDisplay: false});
...
Ext.extend(Ext.grid.CellSelectionModel, Ext.grid.AbstractSelectionModel,  {
    ...
    handleKeyDown : function(e){
        ...
        switch(k){
             ...
             break;
             default:
                if(g.isEditor && !e.isNavKeyPress()){
                    var ch = String.fromCharCode(e.getCharCode());
                    if(ch){
                        this.textBuffer.push(ch);
                        if(!g.editing){
                            g.startEditing(r, c, this.textBuffer);
                        }
                    }
                    return;
                }
