...
(function($) {
  ...
  $.ui.toaster = function(el, settings){
    ...
    var options = $.extend({
      ...
      animationSpeed: 500,
      // callbacks
      show: function(){if(console){console.log(options.name+" is shown");}},
      hide: function(){if(console){console.log(options.name+" is hidden");}}
    }, settings);
    ...
    eval("$('#"+id+"').animate({height: 'toggle'}, "+options.animationSpeed+", null, function(){$('#"+id+"').find('.ui-toast-close').css('display','block'); options.show();});");
