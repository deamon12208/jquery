...
(function($) {
	...
	$.fn.quicksearch = function (options) {
		...
		var form1 	= new $._form(this.settings);
		var key1	= new $._key(this.settings);
		var loader	= new $._loader(this.settings);
		var stripes = new $._stripe(this.settings.stripeRowClass);	
	...
	this.placeForm = function () {
		...
		if(formPosition == 'before') {
			$(formAttached).before( $._makeForm(this.settings) );
		} else if(formPosition == 'prepend') {
			$(formAttached).prepend( $._makeForm(this.settings) );
		} else if(formPosition == 'append') {
			$(formAttached).append( $._makeForm(this.settings) );
		} else {
			$(formAttached).after( $._makeForm(this.settings) );
		}
	};
