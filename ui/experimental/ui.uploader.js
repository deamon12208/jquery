	...
	$.ui.uploader = {
		...
		callback: function(sWhat, sMovieID, vP1, vP2, vP3) {
				...
				if ($.isFunction(fCb)) {
					...
					if ((sWhat == 'swfLoaded') || (sWhat == 'dialogPostShow'))
						oOpt.success = vP1;
		...
		debug: function(oUploader) {
				var oSbDebug = new $.ui.builder("\n");
				oSbDebug.add('----- DEBUG SETTINGS START ----');
				oSbDebug.add('ID: ' + oUploader.opt.sName);
				for (var sKey in oUploader.opt) {
					oSbDebug.add(sKey + ': ' + oUploader.opt[sKey]);
				}
				oSbDebug.add('----- DEBUG SETTINGS END ----');
				$.uploader.log(oSbDebug.toString(), true);
			},
		/**
